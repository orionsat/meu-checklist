...
const photoCategories = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'foto_estepe_pneu', label: 'Foto do Estepe', instruction: 'Fotografe o estepe dentro do porta-malas mostrando o estado do pneu.', exampleImg: '/estepe.png' }
    ]
  }
];
...
export default function ChecklistApp() {
  ...
  const handleFileChange = (e, id) => {
    ...
    if (file) {
      ...
      setFilePreviews(prev => ({ ...prev, [id]: URL.createObjectURL(file) }));
    }
  };
  ...
  const handleReview = (e) => {
    ...
    if (uploadedPhotos < totalPhotos) {
      alert(`Atenção: Você tirou ${uploadedPhotos} de ${totalPhotos} fotos obrigatórias. Por favor, tire todas as fotos, incluindo a do estepe.`);
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Endpoint da sua VPS no n8n (Substituir quando for pra produção)
      const resposta = await fetch('http://localhost:5678/webhook-test/receber-laudo', {
        method: 'POST',
        body: dataToSend,
      });
        ...
        <div className="bg-slate-900 text-white p-6 border-b-[6px] border-blue-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm bg-slate-800 p-4 rounded-lg border border-slate-700">
            ...
            <div>
              ...
              <p>Status: <span className="text-yellow-400 font-bold">Aguardando Envio</span></p>
            </div>
          </div>
        </div>
        ...
        {step === 1 && (
          <form onSubmit={handleReview} className="p-4 md:p-6 space-y-8">
            ...
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-slate-800 border-b-2 pb-2 border-slate-200">Uso do Estepe</h2>
        ...
        {step === 2 && (
          <div className="p-4 md:p-6 space-y-8 animate-fade-in">
            <div className="bg-yellow-50 border-l-4 border-yellow-500 p-5 rounded-r-lg text-yellow-800 shadow-sm">
              ...
              <p className="text-sm mt-2 font-medium">Você está prestes a enviar um documento oficial. Revise se as fotos estão nítidas e as informações estão corretas. <br/>A locadora utilizará este laudo para auditoria da frota.</p>
            </div>
            
            <div className="space-y-4">
              <h3 className="font-black text-xl text-slate-800 border-b-2 pb-2">Suas Fotos Capturadas</h3>
